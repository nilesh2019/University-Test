              ...
              <div class="col-md-12">
                <div class="panel widget">
                  <div class="panel-heading vd_bg-grey">
                    <h3 class="panel-title"> <span class="menu-icon"> <i class="fa fa-dot-circle-o"></i> </span> Manage clients Table</h3>

                  </div>
                  <div class="panel-body table-responsive" style="border: 1px solid #eee;min-height:500px;">
                       <center>
                  <?php $this->load->view('admin/clients/add_view');?>
                  </center>
                  <button id="addClient" onclick="clearFields()" style="float:right;margin-bottom:10px;" class="btn btn-primary " data-target="#myModal" data-toggle="modal">Add New Client</button>

                        <form action="<?php echo base_url();?>admin/client/multiselect_action" method="post" name="myform" id="myform">
...
<script type="text/javascript" src="<?php echo base_url();?>backend_assets/plugins/blockUI/jquery.blockUI.js"></script>
           ...
           return '<div class="panel widget light-widget" style="box-shadow:-2px 5px 17px #ccc !important;">'+
            ...
            '<div class="panel-body">'+
                    ...
                    '<div class="row mgbt-xs-10">'+
                      '<div class="col-xs-5 text-right"> <strong>Client Name</strong> </div>'+
    ...
    var dt = $('#example').DataTable( {
       oLanguage: {
        ...
        "processing": true,
        "serverSide": true,
        "ajax": "<?php echo base_url();?>admin/client/get_ajaxdataObjects",
        "columns": [
            {
            ...
            { "data": "id" },
            { "data": "name" },
            { "data": "logo" },  //columns that you want to show in table
            { "data": "status" },
            { "data": "action" }
        ],
        "order": [],
...
function change_status(userId)
{
      ...
      if(done == true)
      {
          var pageurl_new = '<?php echo base_url();?>'+'admin/client/change_status/'+userId;
...
{
      var done = confirm("Are you sure, you want to delete the client?");
      if(done == true)
      {
          var pageurl_new = '<?php echo base_url();?>'+'admin/client/delete_client/'+userId;
          window.location.href = pageurl_new;
      }
      else
      {
...
function validateForm()
{
 ...
 function clearFields()
     {
                $('#myModal #myModalLabel').text('Add Client');
  ...
  function submitForm(){
                  ...
                  BASEURL='<?php echo base_url();?>';
                             ...
                             // var formData = $( "#login-form" ).serialize();
                              ...
                              $.ajax({
                                  url: BASEURL+"admin/client/processForm",
                                  type: 'POST',
                                  data:  formData,
                                  cache: false,
                              ...
                              }).done(function(responce)
                               {
                                          ...
                                          else
                                          {
                                              if(data.status=='success')
                                              {
                                                      ...
                                                      if(data.for == 'add')
                                                      {
                                                          window.location.href = BASEURL+'admin/client/setFlashdata/add';
                                                      }
                                                      else
                                                      {
                                                          window.location.href = BASEURL+'admin/client/setFlashdata/edit';
                                                      }
      ...
      {
        $('#myModal #myModalLabel').text('Edit Client');
        ...
        $.blockUI({ message: '<div aria-hidden="true" aria-labelledby="myModalLabel" role="dialog" tabindex="-1"><div class="modal-dialog" style="width:300px !important;box-shadow:0 6px 7px #000;"><div class="modal-content"><div class="modal-body" style="padding-bottom:15px !important;"><img src="<?php echo base_url();?>backend_assets/img/loadings.gif"></div></div></div></div>' });
            $.ajax({
              url: '<?php echo base_url();?>admin/client/getEditFormData',
              type: 'POST',
                       ...
                       if(index == 'logo')
                        {
                              ...
                              var img=front_upload_base_url+'client/thumbs/'+val;

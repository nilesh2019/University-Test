              ...
              <div class="col-md-12">
                <div class="panel widget">
                  <div class="panel-heading vd_bg-grey">
                    <h3 class="panel-title"> <span class="menu-icon"> <i class="fa fa-dot-circle-o"></i> </span> Manage Newsletter Table</h3>

                  </div>
                  <div class="panel-body table-responsive" style="border: 1px solid #eee;min-height:500px;">
                       <center>
                  <?php $this->load->view('admin/blog/addedit_view');?>
                  </center>
                  <button id="addblog" style="float:right;margin-bottom:10px;" class="btn btn-primary " data-target="#myModal" data-toggle="modal">Add New Newsletter</button>

                        <form action="<?php echo base_url();?>admin/newsletter/multiselect_action" method="post" name="myform" id="myform">
...
<script type="text/javascript" src="<?php echo base_url();?>backend_assets/plugins/blockUI/jquery.blockUI.js"></script>
    ...
    function format ( d )
    {
           return '<div class="panel widget light-widget" style="box-shadow:-2px 5px 17px #ccc !important;">'+
            ...
            '<div class="panel-body">'+
                    '<span style="font-size:20px;font-weight:bold;margin-left:40%;">Newsletter Detail</span><br><br>'+

                    '<div class="row mgbt-xs-10">'+
                      '<div class="col-xs-5 text-right"> <strong>Newsletter Title</strong> </div>'+
                      '<div class="col-xs-7">  '+d.title+' </div>'+
                    '</div>'+
                    '<div class="row mgbt-xs-10">'+
                      '<div class="col-xs-5 text-right"> <strong>Newsletter Image</strong> </div>'+
                      '<div class="col-xs-7">  '+d.picture+' </div>'+
                    '</div>'+
                    '<div class="row mgbt-xs-10">'+
                      '<div class="col-xs-5 text-right"> <strong>Newsletter Description:</strong> </div>'+
    ...
    var dt = $('#example').DataTable( {
       oLanguage: {
        ...
        "processing": true,
        "serverSide": true,
        "ajax": "<?php echo base_url();?>admin/newsletter/get_ajaxdataObjects",
        "columns": [
            {
            ...
            { "data": "id" },
            { "data": "title" },
            { "data": "picture" },  //columns that you want to show in table
            { "data": "keywords" },
            { "data": "created" },
            { "data": "status" },
            { "data": "action" }
...
function change_status(userId)
{
      ...
      if(done == true)
      {
          var pageurl_new = '<?php echo base_url();?>'+'admin/newsletter/change_status/'+userId;
...
function delete_blog(userId)
{
      var done = confirm("Are you sure, you want to delete the newsletter?");
      if(done == true)
      {
          var pageurl_new = '<?php echo base_url();?>'+'admin/newsletter/delete_blog/'+userId;
          window.location.href = pageurl_new;
      }
      else
      {
   ...
   $( "#datepicker-normal" ).datepicker({ dateFormat: 'dd-M-yy',gotoCurrent: true,minDate: 0});
    ...
    $('#login-form')[0].reset();
         // $(this).find('#tokenize').tokenize().clear();
          ...
          $('.tag').remove();
         $('#myModal #myModalLabel').text('Add New Newsletter');
   })
     ...
     $(document).ready(function() {
             ...
             form_register_2.validate({
                 ...
                 messages: {
                         ...
                         minlength:"Newsletter title must be at least 10 characters in length"
                        },
...
$('#wysiwyghtml').wysihtml5();
  ...
  function submitForm(){
                  ...
                  BASEURL='<?php echo base_url();?>';
                             ...
                             // var formData = $( "#login-form" ).serialize();
                              ...
                              $.ajax({
                                  url: BASEURL+"admin/newsletter/processForm",
                                  type: 'POST',
                                  data:  formData,
                                  cache: false,
                              ...
                              }).done(function(responce)
                               {
                                          ...
                                          else
                                          {
                                              if(data.status=='success')
                                              {
                                                      ...
                                                      if(data.for == 'add')
                                                      {
                                                          window.location.href = BASEURL+'admin/newsletter/setFlashdata/add';
                                                      }
                                                      else
                                                      {
                                                          window.location.href = BASEURL+'admin/newsletter/setFlashdata/edit';
                                                      }
      ...
      function openEditForm(blogId)
      {
        $('#myModal #myModalLabel').text('Edit Newsletter');
        ...
        $.blockUI({ message: '<div aria-hidden="true" aria-labelledby="myModalLabel" role="dialog" tabindex="-1"><div class="modal-dialog" style="width:300px !important;box-shadow:0 6px 7px #000;"><div class="modal-content"><div class="modal-body" style="padding-bottom:15px !important;"><img src="<?php echo base_url();?>backend_assets/img/loadings.gif"></div></div></div></div>' });
            $.ajax({
              url: '<?php echo base_url();?>admin/newsletter/getEditFormData',
              type: 'POST',
              data: {blogId:blogId},
            })
            .done(function(responce) {
                  ...
                  else
                  {
                          ...
                          else
                          {
                                if(index == 'picture')
                                {
                                      ...
                                      var img='<?php echo file_upload_base_url();?>blog/thumb/'+val;
  ...
  $(document).ready(function() {
   ...
   $('#addblog').click(function()
       {
          $('.vd_input-wrapper light-theme .wysihtml5-toolbar .btn btn-default').remove();
       });

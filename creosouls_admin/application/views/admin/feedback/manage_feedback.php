							...
							<div class="col-md-12">
								<div class="panel widget">
									...
									<div class="panel-body table-responsive" style="border: 1px solid #eee;min-height:500px;">
										...
										<form action="<?php echo base_url();?>admin/feedback_instance/multiselect_action" method="post" name="myform" id="myform">
...
<script type="text/javascript" language="javascript" >
	function format ( d )
	{
		...
		'<span style="font-size:20px;font-weight:bold;margin-left:40%;">Feedback Instance Details</span><br><br>'+
		'<div class="row mgbt-xs-10">'+
		'<div class="col-xs-5 text-right"> <strong>Name</strong> </div>'+
		'<div class="col-xs-7">  '+d.name+' </div>'+
		'</div>'+
		'<div class="row mgbt-xs-10">'+
		'<div class="col-xs-5 text-right"> <strong>Start Session</strong> </div>'+
		'<div class="col-xs-7">  '+d.start_session+' </div>'+
		'</div>'+
		'<div class="row mgbt-xs-10">'+
		'<div class="col-xs-5 text-right"> <strong>End Session</strong> </div>'+
		'<div class="col-xs-7">'+d.end_session+'</div>'+
		'</div>'+
		'<div class="row mgbt-xs-10">'+
			...
			var dt = $('#example').DataTable(
				{
					...
					"processing": true,
					"serverSide": true,
					"ajax": "<?php echo base_url();?>admin/feedback_instance/get_ajaxdataObjects",
					"columns": [
						{
	...
	function change_status(feedbackInstanceId)
	{
		var done = confirm("Are you sure, you want to change the status?");
		if(done == true)
		{
			var pageurl_new = '<?php echo base_url();?>'+'admin/feedback_instance/change_status/'+feedbackInstanceId;
	...
	function delete_confirm(feedbackInstanceId)
	{
		var done = confirm("Are you sure to delete this record");
		if(done == true)
		{
			var pageurl_new = '<?php echo base_url();?>'+'admin/feedback_instance/delete_confirm/'+feedbackInstanceId;
			window.location.href = pageurl_new;
		}
		else
		{
	...
	$(document).ready(function()
		{
			...
			$('#addInstitute').click(function()
				{
					$('#myModalLabel').html('Feedback Instance Add');
					$('#sessionName').val('');
					$('#sessionStartDate').val('');
					$('#sessionEndDate').val('');
					$('#feedbackInstanceInd').val('');
					$('.vd_red').remove();
			...
			$( "#sessionStartDate" ).datepicker({
				...
				onSelect: function (selected) {
                    ...
                    $("#sessionEndDate").datepicker("option", "minDate", dt);
                }
            });
      		$( "#sessionEndDate" ).datepicker({
				minDate: 0,
      			onSelect: function (selected) {
                    var dt = new Date(selected);
                    dt.setDate(dt.getDate());
                    $("#sessionStartDate").datepicker("option", "maxDate", dt);
                }
      		});
		});
</script>
...
<script>
	$(document).ready(function()
		{
			"use strict";
			var feedbackInstanceForm = $('#feedbackInstanceForm');
			var error_feedbackInstanceForm = $('.alert-danger', feedbackInstanceForm);
			var success_feedbackInstanceForm = $('.alert-success', feedbackInstanceForm);

			feedbackInstanceForm.validate(
				{
					...
					messages:
					{
						sessionNmae: "Session name is required",
						sessionStartDate:
						{
					...
					invalidHandler: function (event, validator)
					{
						...
						success_feedbackInstanceForm.hide();
						error_feedbackInstanceForm.show();
					},
					...
					{
						$(form).find('#feedback-instance-submit').prepend('<i class="fa fa-spinner fa-spin mgr-10"></i>')/*.addClass('disabled').attr('disabled')*/;
						//success_feedbackInstanceForm.show();
						//error_feedbackInstanceForm.hide();
						submitForm();
					}
				});
		});
	...
	function submitForm()
	{
		...
		$('#feedback-instance-submit').prop("disabled", true);
		var feedbackInstanceForm = $('#feedbackInstanceForm');
		var error_feedbackInstanceForm = $('.alert-danger', feedbackInstanceForm);
		var success_feedbackInstanceForm = $('.alert-success', feedbackInstanceForm);
		// var formData = $( "#feedbackInstanceForm" ).serialize();
		var formElement = document.getElementById("feedbackInstanceForm");
		var formData=new FormData(formElement);
		$.ajax(
			{
				url: BASEURL+"admin/feedback_instance/processForm",
				type: 'POST',
				data:  formData,
				cache: false,
...

			{
				...
				if(data.status=='error')
				{
					...
					$('#feedback-instance-submit').prop("disabled", false);
				}
				else
				{
					if(data.status=='success')
					{
						document.getElementById("feedbackInstanceForm").reset();

						if(data.for == 'add')
						{
							window.location.href = BASEURL+'admin/feedback_instance/setFlashdata/add';
						}
						else
						{
							window.location.href = BASEURL+'admin/feedback_instance/setFlashdata/edit';
						}
					}
					else
					{
	...
	function openEditForm(feedbackInstanceId)
	{
		...
		$.ajax(
			{
				url: '<?php echo base_url();?>admin/feedback_instance/getEditFormData',
				type: 'POST',
				data:
				{
					feedbackInstanceId:feedbackInstanceId
				},
			})
		...
		.done(function(responce)
			{
				...
				$('#feedbackInstanceId').val(feedbackInstanceId);
				$('#myModal').modal('show');
				//$('#myModalLabel').html('');
				$('#myModalLabel').html('Feedback Instance Edit');
				$.unblockUI();
			})
		.fail(function()
			{

              ...
              <div class="col-md-12">
                <div class="panel widget">
                  <div class="panel-heading vd_bg-grey">
                    <h3 class="panel-title"> <span class="menu-icon"> <i class="fa fa-dot-circle-o"></i> </span> Manage Admin Table</h3>
                  </div>
                  <div class="panel-body table-responsive" style="border: 1px solid #eee;min-height:500px;">
                    <center>
                     <!--   <a href="<?php echo base_url();?>admin/Admin/exportAdmin" class="btn btn-info" style="margin-top: -2px;cursor:pointer;"><i class="icon-export"> <span style="font-family:'Open Sans','arial'">Export All Admin To CSV</span></i></a>
                      ...
                      <?php $this->load->view('admin/admin/addedit_view');?>
                    </center>

                        <form action="<?php echo base_url();?>admin/admin/multiselect_action" method="post" name="myform" id="myform">
...
<script type="text/javascript" src="<?php echo base_url();?>backend_assets/plugins/bootstrap-wysiwyg/js/bootstrap-wysihtml5-0.0.2.js"></script>
    ...
    var dt = $('#example').DataTable( {
       oLanguage: {
        ...
        "processing": true,
        "serverSide": true,
        "ajax": "<?php echo base_url();?>admin/admin/get_ajaxdataObjects",
        "columns": [
            {
...
function change_status(Id)
{
      ...
      if(done == true)
      {
          var pageurl_new = '<?php echo base_url();?>'+'admin/admin/change_status/'+Id;
...
function delete_confirm(userId)
{
      ...
      if(done == true)
      {
          var pageurl_new = '<?php echo base_url();?>'+'admin/admin/delete_confirm/'+userId;
          window.location.href = pageurl_new;
      }
      else
      {
...
$(document).ready(function()
{
  ...
  {   
    BASEURL='<?php echo base_url();?>';
       var pageurl_new = BASEURL+'admin/admin/edit_admin/'+Id;
       $.ajax({
          url: pageurl_new,
       }).done(function(responce)
       {
        ...
        if (!$.isEmptyObject(responce))
        {
           ...
           var $radios = $('input:radio[name=manage_user]');
           if($radios.is(':checked') === false)
           {
               $radios.filter('[value='+data.manage_user+']').prop('checked', true);
           }
           else
           {
             $radios.filter('[value='+data.manage_user+']').attr('checked', true);
           }
        }
       });
   }

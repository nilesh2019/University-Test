              ...
              <div class="col-md-12">
                <div class="panel widget">
                  <div class="panel-heading vd_bg-grey">
                    <h3 class="panel-title"> <span class="menu-icon"> <i class="fa fa-dot-circle-o"></i> </span> Manage Assignment Table</h3>
                  </div>
                  <div class="panel-body table-responsive" style="border: 1px solid #eee;min-height:500px;">
                    <center>
                      ...
                      <?php $this->load->view('admin/assignment/addedit_view');?>
                    </center>

                        <form action="<?php echo base_url();?>admin/assignment/multiselect_action" method="post" name="myform" id="myform">
...
<script type="text/javascript" src="<?php echo base_url();?>backend_assets/plugins/bootstrap-wysiwyg/js/bootstrap-wysihtml5-0.0.2.js"></script>
    ...
    function format ( d )
    {
           ...
           return '<div class="panel widget light-widget" style="box-shadow:-2px 5px 17px #ccc !important;">'+
            ...
            '<div class="panel-body">'+
                    '<span style="font-size:20px;font-weight:bold;margin-left:40%;">Assignment Template Detail</span><br><br>'+                   
                    '<div class="row mgbt-xs-10">'+
                      '<div class="col-xs-5 text-right"> <strong>Assignment Name</strong> </div>'+
                      '<div class="col-xs-7">  '+d.name+' </div>'+
                    '</div>'+
                    '<div class="row mgbt-xs-10">'+
                      '<div class="col-xs-5 text-right"> <strong>Product Name</strong> </div>'+
                      '<div class="col-xs-7">  '+d.product_name+' </div>'+
                    '</div>'+
                    '<div class="row mgbt-xs-10">'+
    ...
    var dt = $('#example').DataTable( {
       oLanguage: {
        ...
        "processing": true,
        "serverSide": true,
        "ajax": "<?php echo base_url();?>admin/assignment/get_ajaxdataObjects",
        "columns": [
            {
...
function change_status(Id)
{
      ...
      if(done == true)
      {
          var pageurl_new = '<?php echo base_url();?>'+'admin/assignment/change_status/'+Id;
...
function delete_confirm(userId)
{
      ...
      if(done == true)
      {
          var pageurl_new = '<?php echo base_url();?>'+'admin/assignment/delete_confirm/'+userId;
          window.location.href = pageurl_new;
      }
      else
      {
...
$(document).ready(function()
{
  ...
  function submitForm()
  {
    ...
    BASEURL='<?php echo base_url();?>';
          ...
          $.ajax({
              url: BASEURL+"admin/assignment/add",
              type: 'POST',
              data:  formData
          }).done(function(responce)
           {
              ...
              else
              {
                  if(data.status=='success')
                  {
                         ...
                         if(data.for == 'add')
                                    {
                                      window.location.href = BASEURL+'admin/assignment/setFlashdata/add';
                                    }
                                    else
                                    {
                                      window.location.href = BASEURL+'admin/assignment/setFlashdata/edit';
                                    }
                  }
                  else
                  {
  ...
  {   
    BASEURL='<?php echo base_url();?>';
       var pageurl_new = BASEURL+'admin/assignment/edit_assignment/'+Id;
       $.ajax({
          url: pageurl_new,
       }).done(function(responce)
       {
           ...
           var data = jQuery.parseJSON(responce);
            $.each(data, function(index, val)
              {           
                if(index == 'tools')
                {
                  $.each(val, function(toolindex, toolval) {
                    
                    $('select#tool option[value='+toolval+']').attr('selected','selected');
                  });                             
                                       
                }
                });
           $('[name*="id"]').val(data.id);  
           $('[name*="name"]').val(data.assignment_name);

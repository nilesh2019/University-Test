          ...
          <div class="vd_head-section clearfix">
            <div class="vd_panel-header">
              <ul class="breadcrumb">
                ...
                <li class="active">Manage Competition</li>
              </ul>
              ...
              <div class="col-md-12">
                <div class="panel widget">
                  <div class="panel-heading vd_bg-grey">
                    <h3 class="panel-title"> <span class="menu-icon"> <i class="fa fa-dot-circle-o"></i> </span> Manage Competition Table</h3>
                  </div>
                  <div class="panel-body table-responsive" style="border: 1px solid #eee;min-height:500px;">
                    <center>
                      <?php $this->load->view('admin/competition/addedit_student');?>
                      <?php $this->load->view('admin/competition/ratings_view');?>
                    </center>
                      <button id="addInstitute" style="float:right;margin-bottom:10px;" class="btn btn-primary" onclick="getAutocomplete();"  data-toggle="modal" data-target="#myModal">Add Student To Competition</button>
                        <form action="<?php echo base_url();?>admin/competition/multiselect_action" method="post" name="myform" id="myform">
    ...
    <div class="modal fade" id="avatar-modal" aria-hidden="true" aria-labelledby="avatar-modal-label" role="dialog" tabindex="-1">
      <div class="modal-dialog modal-lg">
        <div class="modal-content">
          <form class="avatar-form" action="<?php echo base_url();?>admin/competition/upload_image" enctype="multipart/form-data" method="post">
...
/*    function reposition() {
    ...
    $('#ratingsModal').on('show.bs.modal', reposition);
    ...
    $(window).on('resize', function() {
      ...
      $(document).on("click", "#viewRating", function ()
      {
           ...
           $.ajax({
            url: '<?php echo base_url();?>'+'admin/competition/getRating',
            type: 'POST',
    ...
    $(document).ready(function() {
        ...
        $('#tokenize').tokenize({
          newElements:false,
          datas: base_url+'admin/competition/getCompetitionJury',
            ...
            '<div class="panel-body">'+
                  ...
                  '</div><div class="panel-body"><div class="col-md-12"><table id="example2" class="example2 display" cellspacing="0" width="100%">'+
                          ...
                          '<thead style="background: #0c99d5 none repeat scroll 0 0 !important;">'+
                            ...
                            '<tbody>'+d.competition_students+'<tbody>'+
...
/*  '<div class="row mgbt-xs-10">'+
    ...
    var dt = $('#example').DataTable( {
       oLanguage: {
        ...
        "processing": true,
        "serverSide": true,
        "ajax": "<?php echo base_url();?>admin/competition/get_studentDataObjects",
        "columns": [
            {
...
function change_status(userId)
{
      ...
      if(done == true)
      {
          var pageurl_new = '<?php echo base_url();?>'+'admin/competition/change_status/'+userId;
...
function delete_confirm(userId)
{
      ...
      if(done == true)
      {
          var pageurl_new = '<?php echo base_url();?>'+'admin/competition/delete_confirm/'+userId;
          window.location.href = pageurl_new;
      }
      else
      {
...
function emailToJury(compId)
{
  ...
  if(done == true)
  {
      var pageurl_new = '<?php echo base_url();?>'+'admin/competition/emailToJury/'+compId;
      window.location.href = pageurl_new;
  }
  else
  {
...
function declareResult(compId)
{
  ...
  if(done == true)
  {
      var pageurl_new = '<?php echo base_url();?>'+'admin/competition/declareResult/'+compId;
      window.location.href = pageurl_new;
  }
  else
  {
  ...
  <![endif]-->
       ...
       $(document).ready(function() {
               ...
               registerInstituteForm.validate({
                   ...
                   messages: {
                           ...
                           competition: "Competition name is required",
                           
                           zone: "Competition Start date is required",
                           region: "Competition Evaluation Start date is required",
                           institute: "Award is required",
                           student: "Eligibility is required"
    ...
    function submitForm(){
                    BASEURL='<?php echo base_url();?>';
                               ...
                               // var formData = $( "#instituteRegistrationForm" ).serialize();
                                ...
                                $.ajax({
                                    url: BASEURL+"admin/competition/linkStudent",
                                    type: 'POST',
                                    data:  formData,
                                    cache: false,
                                ...
                                }).done(function(responce)
                                 {
                                            ...
                                            else
                                            {
                                                if(data.status=='success')
                                                {
                                                        ...
                                                        if(data.for == 'add')
                                                        {
                                                            window.location.href = BASEURL+'admin/competition/linkStudent/setFlashdata/add';
                                                        }
                                                        else
                                                        {
                                                            window.location.href = BASEURL+'admin/competition/linkStudent/setFlashdata/edit';
                                                        }
                                                }
                                                else
                                                {
    ...
    function submitJuryForm(){
                    BASEURL='<?php echo base_url();?>';
                               ...
                               // var formData = $( "#instituteRegistrationForm" ).serialize();
                                ...
                                var formData=new FormData(formElement);
                                $.ajax({
                                    url: BASEURL+"admin/competition/processJuryForm",
                                    type: 'POST',
                                    data:  formData,
                                    cache: false,
                                    processData: false, // Don't process the files
      ...
      function openEditForm(competitionId)
      {
        $('#myModal #myModalLabel').text('Edit Competition');
            ...
            $.ajax({
            url: '<?php echo base_url();?>admin/competition/getEditFormData',
            type: 'POST',
            ...
            $.each(data, function(index, val)
            {
                ...
                else
                {
                    ...
                    else if(index == 'video_link')
                    {
                        $("#video_link").val(val);
                    } 
                    else if(index == 'pdf_file')
                    {
                        $("#pdf_file").val(val);
                    }
                    else if(index == 'award')
            ...
            else if(index=='winnerEmail')
            {
              ...
              $('#wysiwyghtml').data("wysihtml5").editor.setValue(val);
                    ...
                    if(index == 'competition_type')
                    {
                        console.log(val);

                        if(val == '2'){
                          var th =  '<option value="2" selected>Team Competition</option>';
                        } else{

                          var th = '<option value="1" selected>Single Competition</option>'
    ...
    function getAutocomplete()
    {
            <?php if($this->session->userdata('admin_level') == 1){;?>
              var base_url='<?php echo front_base_url();?>';
              $.ajax({
                url: base_url+'creosouls_admin/admin/competition/getAutocompleteInstituteData',
                type: 'POST',
               })
...
function getCities(id)
{
  ...
  url:base_url+'admin/competition/getAllCities/'+countryId,
...
<script type="text/javascript">
  function getRegionList(element)
  {
      ...
      $.ajax({
            ...
            url: base_url+'creosouls_admin/admin/competition/getZoneRegionList',
            success:function(data)
            {
              if(data!='')
              {
    ...
    function getInstituteList(element){
      ...
      $.ajax({
            ...
            url: base_url+'creosouls_admin/admin/competition/getRegionInstituteList',
            success:function(data)
            {
              if(data!='')
              {
    ...
    function getStudentList(element){
          ...
          $.ajax({
                ...
                url: base_url+'creosouls_admin/admin/competition/getInstituteStudentList',
                success:function(data)
                {
                  if(data!='')
                  {

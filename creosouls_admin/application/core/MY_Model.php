...
class MY_Model extends CI_Model
{
	...
	//  to fetch the action of login user
	function fetchActionsForLoginUser($id)
	{
	  $this->db->select("role_action_relation.*,action_master.*");
	  $this->db->from("role_action_relation");
	  $this->db->where("role_action_relation.tenant_id",$this->session->userdata('tenant_id'));
	  $this->db->where("role_action_relation.role_id",$id);
	  $this->db->join('action_master', 'action_master.id = role_action_relation.action_id', 'left');
	  return $this->db->get()->result_array();
	}
	
	
	function limitedActionsForUser()
	{
	  $arr = array();
	  $user_actions = $this->fetchActionsForLoginUser($this->session->userdata('role_id'));
		  if(!empty($user_actions))
		  {
		  	 foreach($user_actions as $val)
			 {
			 	$arr[] = $val['action_name'];
			 }
		  }  
	...
	function getPhraseByLabel($phrase)
	{
	...
	{
		$language=$this->session->userdata('language');
		$this->db->select("phrase,$language");
		$this->db->from("language");

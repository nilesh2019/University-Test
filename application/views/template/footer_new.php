...
<!-- Google Drive Setting -->
<footer style="z-index: 999999 !important">
        	...
        	<div class="col-lg-12">
            	<ul class="pull-left col-lg-6">
            	 <li>
            	 	 ...
            	 	 <!--<span style="margin-top: 40px;position: absolute;margin-left: -200px;">Powered By Creonow</span>-->
 ...
 ?>
	...
	function checkStudentId()
	{
			...
			var maac_url = '<?php echo maac_base_url();?>';
     		var lakme_url = 'https://www.creosouls.com/lakme/';
			if(studentId != '')
			{
				$.ajax({
				...
				.done(function(data)
				{
					...
					else if(data==11 || data==44)
					{
						//alert('dghf');
						document.getElementById('myForm').reset();
						//window.location.replace(maac_url+'hauth/setstudentid/'+studentId);
						//window.location.replace(maac_url+'home/checkStudentId/'+studentId);
						$.ajax({url: maac_url+'hauth/setstudentid/'+studentId, success: function(result){
							//alert(result);
						        window.location=base_url+'hauth/googleLogin';
						    }});
						//window.location=base_url+'hauth/googleLogin/'+studentId;
						return false;
					}
					else if(data==333 || data==666)
			          {
			            //alert(data+'---3333');
			            document.getElementById('myForm').reset();
			            //window.location.replace(maac_url+'hauth/setstudentid/'+studentId);
			            //window.location.replace(maac_url+'home/checkStudentId/'+studentId);
			            /*$.ajax({url: lakme_url+'hauth/setstudentid/'+studentId, success: function(result){
			              //alert(result);
			                    window.location=base_url+'hauth/googleLogin';
			                }});*/
			                window.location=(lakme_url+'hauth/setstudentid/'+studentId);
			            //window.location=base_url+'hauth/googleLogin/'+studentId;
			            return false;
			          }
					else
					{
						...
						else if(data == 3)
						{
							...
							{
								$('#myForm')[0].reset();
								$('#myForm span.text-danger').html('No user found with this student ID, Check your Student ID  is correct and try again: for example: Student00000 .');
								return false;
							}
							else if (countClicks>='2')
							{
								$('.student-div1').css("display", "block");
								$('#myForm')[0].reset();

								$('#myForm span.text-danger').html(' If you are still having issues: Click below link to fill out the below form.');
								return false;
							}
	...
	$('.student-div1').on('click',function(e){
		$('.student-div').css("display", "block");
	});
	function getinsdata()
	{
		var ins_type = $("input[name='categorytype']:checked").val();
		//alert(ins_type);
		if(ins_type=='Arena')
		{
			$(".ARENA").css('display','block');
			$(".MAAC").css('display','none');
			$(".LAKME").css('display','none');
		}
		else if(ins_type=='Maac')
		{
			$(".MAAC").css('display','block');
			$(".ARENA").css('display','none');
			$(".LAKME").css('display','none');
		}
		else{
			$(".LAKME").css('display','block');
			$(".ARENA").css('display','none');
			$(".MAAC").css('display','none');
		}
	}

	function sendemaildtosupport()
	{
		var base_url = $('#base_url').val();
		var studentId = $('#studentId').val();
		document.getElementById("supportformbutton").disabled = true;
		//var serialize_data = $('#myForm1').serialize();
		//alert(serialize_data);
		var form = $('#myForm1')[0];
		var formData = new FormData(form);
		$.ajax({
			url: base_url+'home/sendemaildtosupport',
			type: 'POST',
			processData: false,
			contentType: false,
			data: formData,
		...
		.done(function(data)
		{
			...
			if(data1=='Success')
			{
				var messagetext='Your message has been sent to Creosouls Support Team. Please note down the reference number '+data2+' You will receive a response from Creosouls Support Team within 24 hours. Kindly check your Email. ';
				//var messagetext1=messagetext.concat(data2);
				$('#myForm1')[0].reset();
				//document.getElementById("myForm1").style.display="none";
				$('#myInstituteModal').modal('hide');
				$('#message_model').modal('show');
				$('#message_model').find('div.message_show').html(messagetext);

				return true;
			}
			else if(data1=='Error')
			{
				var messagetext='You message not sent, Please try after some time. ';
				var messagetext1=messagetext.concat(data2);
				//$('#myForm1 span.text-danger').html(messagetext1);
				$('#myForm1')[0].reset();
				$('#myInstituteModal').modal('hide');
				$('#message_model').modal('show');
				$('#message_model').find('div.message_show').html(messagetext1);

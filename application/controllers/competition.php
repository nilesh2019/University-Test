...
class Competition extends CI_Controller
{
    ...
    if(is_array($_POST['studentTeam'])){
     ...
     public function saveSortProject()
		{
			$projId = $_POST['proId'];
			$compId = $_POST['compId'];
			$userId = $_POST['juryId'];
			...
			{
				$existIn = $this->competition_model->checkInSaveProject($_POST['proId'],$_POST['compId'],$_POST['juryId']);
		
		    	if(empty($existIn))
		    	{
					$result = $this->competition_model->saveSortProject($_POST['proId'],$_POST['compId'],$_POST['juryId']);
					if($result > 0)
					{
						echo 'done';
					}
		...
		public function spotlightProjects($competitionId='',$juryId=''){
			if($this->input->post('sort_projet',true)){
			...
			if($this->input->post('reset_projet',true)){
					
				$this->session->unset_userdata('catid');
			 
			}
			$catid = $this->session->userdata('catid');
			$data['project']=$this->competition_model->getAllSortedProjects($competitionId,$juryId,$catid);
			$data['totalProjectCount']=count($data['project']);
			$data['category']=$this->competition_model->getAllCategory();
	    	$this->load->view('spotlight_project_sort_view',$data);
		}
		public function removeFromSpotlight(){
			$projId = $_POST['proId'];
			$compId = $_POST['compId'];
			$userId = $_POST['juryId'];
			if(isset($_POST['proId'])&&$_POST['proId']!='')
			{
		    	...
		    	if(!empty($existIn))
		    	{
					$result = $this->competition_model->removeFromSpotlight($_POST['proId'],$_POST['compId'],$_POST['juryId']);
					if($result > 0)
					{
						echo 'done';
					}

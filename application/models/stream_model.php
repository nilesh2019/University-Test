...
public function getFollowingUsersLikedProjects($uids, $arStart, $myfollwing = '')
	{
		...
		if($myfollwing && $myfollwing == 2){

			if(!empty($pid)){
				$piUids = implode(',', array_unique(array_column($pid, 'projectId')));
				$where = '(project_master.id IN ('.$piUids.')) ';
		...
		} elseif ($myfollwing && $myfollwing == 1) {
			//print "am hree";
			//$piUids = implode(',', array_unique(array_column($pid, 'projectId')));
				//$whr = ' AND (project_master.id IN ('.$piUids.')) ';
			$where = '(project_master.featured = 1) ';
		} else {

		if(!empty($pid)){
				$piUids = implode(',', array_unique(array_column($pid, 'projectId')));
				$whr = ' OR (project_master.id IN ('.$piUids.')) ';
		} else{
			$whr = '';
		}
		...
		$where = '(project_master.featured = 1) '.$whr;
...
//      	  if(in_array($value['id'], array_unique(array_column($pid, 'projectId')))){
//      	  	$newData[] = $value;
//      	  }
//      }
 ...
 * @return [type]                [description]
 */
	function timeline_more_data($arStart,$arEnd,$timelIneArray, $sort){
  ...
  $currentUserLikeedUsers = array_column($followingUsers, 'followingUser');
		...
		$getFollowingUsersLikedProjects = $this->getFollowingUsersLikedProjects($currentUserLikeedUsers, $arStart, $sort);
     ...
     //   	$featureds = 1;
     //     $finalListOfProject = array_filter($finalListOfProject, function ($var) use ($featureds) {
     ...
     if(isset($sort) && $sort == 2){
         ...
         $finalListOfProject = $this->getFollowingUsersLikedProjects($currentUserLikeedUsers, $arStart, 2);
          $lcomments = array();
              foreach ($finalListOfProject as $key => $row)
              {
                //$price[$key]['name'] = $row->name;
...
?>
      ...
      <div class="post-bar project-custom-bar ajax-custompost-bar" id="div_<?php echo $i;?>">
